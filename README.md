...
- [JHT(ホスト連携ツール)SIMPLE版](http://www.vector.co.jp/soft/winnt/util/se094205.html)： Windows-31J の範囲は概ねこのツールからマッピングを生成しています。ただし、Windows-31J は 83JIS をベースにしているため、Unicode へのマッピングには不適当な部分があります。jef4j では、Unicode に字形重視で マッピングするよう修正を加えています。
...
JEF標準漢字領域は、78JIS に準拠しているためさほど問題はないのですが、JEF拡張漢字領域に関しては第三版で追加された文字の情報が入手できておらず、完全にマッピング出来ているかわからない状態です。必要なマッピングをご連絡いただければ登録したいと思いますので、Issue や Pull Request にてご連絡をお願いします。
