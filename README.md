...
残念なことに JEF コード体系は、富士通株式会から正式に公開されたものがありません。そのため、下記の文献およびインターネット上で手に入るツールを元に作成しました。
...
- FACOM JEF 文字コード索引辞書 (1980/第二版): 拡張漢字についてはこの資料を元にしています。現在では入手困難なため、図書館にて（書籍貸出不可のため）閲覧および著作権法で許可された範囲でコピーを取得したものを参照し、独自で Unicode へのマッピングを行っています。最新は1987年発売の第三版ですが、福岡大学図書館にしか置かれておらず、現時点では参照できていません。
...
JEF標準漢字領域は、78JIS に準拠しているためさほど問題はないのですが、JEF拡張漢字領域に関しては第３版で追加された文字の情報が入手できておらず、完全にマッピング出来ているかわからない状態です。必要なマッピングをご連絡いただければ登録したいと思いますので、Issue や Pull Request にてご連絡をお願いします。
...
JEF漢字コード体系自体は 78JIS(JIS C 6226:1978)に基づく JIS非漢字、JIS第一水準、第二水準からなる標準のコード域と拡張漢字、拡張非漢字と呼ばれる追加のコード域からなる 2 バイトのコード体系です。
...
PC で一般的に使われる SHIFT_JIS や EUC-JP なども JIS コード体系に基づいてるため標準漢字/非漢字領域に関しては概ね互換性があるのですが、 これらの文字コードは 83JIS(JIS X 0208:1983) に基づいているため、一部の字体やコード域に違いがあります。78JIS から 83JIS での字体変更に際し、その翌年に策定された JEF84 では、従来のコード値はそのままに、変更後の字体は拡張漢字領域に配置されているため上位互換性が保たれています（ただし、79JEFの時点で83JISと同一字体になっている文字があり、差分だけが追加される形になっているようです）。

|JEF|JIS|
|-----|------|
|JEF79|78JIS + 拡張漢字/非漢字|
|JEF84|78JIS + 拡張漢字/非漢字 + 83JIS追加変更字体|
...
例えば、JEF では、二点しんにょうの「辻&#E0102;(C4D4)」と一点しんにょうの「辻&#E0103;(67A5)」が別のコードとして登録されていますが、Unicode ではいずれも「辻（U+8FBB）」にマッピングされます。異体字セレクタを用いると、「辻&#E0102(U+8FBB\_E0102)」および「辻&#E0103(U+8FBB\_E0103)」という形で意図した字形で表示することができるようになります。
...
- 2018/3/11 version 0.7.1: 字形に対する Unicode 値が不明であった２文字（JEF:6AC6、JEF:48C2）が CJK 統合漢字拡張 F に存在することがわかったためマッピング先を修正しました。
